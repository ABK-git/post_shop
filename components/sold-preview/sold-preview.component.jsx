...
import {
  ...
  TextRight,
  WhiteBackground,
} from "./sold-preview.styles";
import router from "next/router";

const SoldPreview = ({ order }) => {
  //context
  const my_context = useContext(MyContext);
  const { hmBreakPoint } = my_context;

  const handleClickTo = () => {
    router.push(`/product/${order.product._id}/update`);
  };
      ...
      <WhiteBackground>
        ...
        <TextRight onClick={handleClickTo}>
          <div>
            ...
            <LeftJutifyStart>売却数：{order.quantity}</LeftJutifyStart>
          </div>
        </TextRight>
        <BorderPrice />
        <AmountPrice>
